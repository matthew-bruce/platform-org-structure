    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          {!isNormal && (
            <div className={`font-bold text-rm-black truncate ${isCompact ? 'text-xs' : isDetailed ? 'text-base' : 'text-sm'}`}>
              {person.name}
            </div>
          )}
          ...
          {!isCompact && (
            <div className="flex flex-wrap gap-1.5 mt-2">
              ...
              <span 
                className={`inline-block px-2 py-0.5 rounded text-white font-bold uppercase ${isDetailed ? 'text-[10px]' : 'text-[9px]'}`}
              ...
              {isDetailed && (
                <>
                  {/* Location badge - grey styling like other metadata */}
